  ...
  if(event != null)
	...
	if(method.toUpperCase() == "POST"){
		...
		$.post(
			...
			{"offset": url_params.get("offset") || 0, "locale": url_params.get("locale"), "sort_c[]": url_params.getAll("sort_c[]"), "sort_d[]": url_params.getAll("sort_d[]"), "filt_fo[]": url_params.getAll("filt_fo[]"), "filt_v[]": url_params.getAll("filt_v[]")},
...
function highlightSort(){
	...
	for(var i = 0; i < f_fields.length; i++){
		...
		document.querySelector("[data-tipo=filtro][data-field=" + fo[0] + "][data-op=" + fo[1] + "]").value = decodeURIComponent(f_vals[i].replace(/\\\\/g,"\\"));
	}
...
function styleDatepickers(){
	$(".fechas").datepicker({
		format: "yyyy-mm-dd", language: "es-MX", autoclose: true, todayBtn: "linked", todayHighlight: true, clearBtn: true
	});
}
