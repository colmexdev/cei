...
function extendTrix(ev){
  ...
  textElement.insertAdjacentHTML("beforeend",'<button type="button" class="trix-button trix-button-icon" tabindex="-1" data-trix-attribute="sub"><sub>SUB</sub></button>');
	...
	element.addEventListener("trix-focus", function(event){
		if(event.target.editor.attributeIsActive("sup") && atributos.has("sub"))
			event.target.editor.deactivateAttribute("sub");
		else if(event.target.editor.attributeIsActive("sub") && atributos.has("sup"))
			event.target.editor.deactivateAttribute("sup");
		actualizarAtributos();
	});
