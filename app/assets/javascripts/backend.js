...
function hideLink(event,element,link,method,keyword,query,sort_el){
  ...
  if(event != null)
	...
	$(element).append('<a ' + (method == "DELETE" ? ('data-method="'+method+'" rel="nofollow" data-remote=true data-confirm="¿Seguro que desea eliminar el objeto?"') : (method == "PUT" ? ('data-method="'+method+'"') : "data-remote=true")) + ' href="' + link + (query_sort === "" ? "&" + query_sort : "") + (keyword != null ? '&keyword='+keyword : "") + (query[0] !== "" ? "&"+query[1]+"&complement="+query[0] : "") +'" style="display:none;" id="vlink"></a>');
