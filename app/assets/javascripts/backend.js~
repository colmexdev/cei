...
function extendTrix(ev){
  ...
  var textElement = toolbarElement.querySelector(".trix-button-group--text-tools");
	...
	textElement.insertAdjacentHTML("beforeend", '<button type="button" class="trix-button trix-button-icon" data-trix-attribute="sup" data-trix-action="x-sup"  tabindex="-1"><sup>SUP</sup></button>');
  textElement.insertAdjacentHTML("beforeend",'<button type="button" class="trix-button trix-button-icon" tabindex="-1" data-trix-attribute="sub" data-trix-action="x-sub"><sub>SUB</sub></button>');
	...
	blockElement.insertAdjacentHTML("beforeend","<button type=\"button\" class=\"attach\" data-action=\"x-attach\"><div style=\"display:inline-block;font-size:10px;width:100%;text-align:center;\"><i class=\"fa fa-cloud-upload\" aria-hidden=\"true\"></i></div></button>");
