class PanelController < ApplicationController
  ...
  def index
    ...
    if params.key?(:filt_fo) && params.key?(:filt_v)
      ...
      @filter_query = campos.zip(ops,vals).map {|a| (@models.columns_hash[a[0]].type == :text ? ("unaccent(" + a[0] + ")") : a[0]) + a[1] + (a[1] == " ilike " ? ("'%" + a[2].downcase.gsub(/(\\)*([%_])?/,'\1\1\\\2') + "%'") : (a[1] == " = " ? ("'" + a[2].downcase.gsub("\\","\\\\") + "'") : (@models.columns_hash[a[0]].type == :date ? ("to_date('" + a[2] + "','YYYY-MM-DD')") : a[2]))) }.join(" AND ")
